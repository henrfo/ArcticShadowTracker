...
class VesselDetector:
    ...
    def find_dark_vessels(self, sar_detections: List[Dict], ais_data: List[Dict], 
        ...
        for sar_detection in sar_detections:
            if not self._has_matching_ais(sar_detection, ais_data, time_tolerance_minutes):
                ...
                if 'latitude' in dark_vessel and 'lat' not in dark_vessel:
                    dark_vessel['lat'] = dark_vessel['latitude']
                if 'longitude' in dark_vessel and 'lon' not in dark_vessel:
                    dark_vessel['lon'] = dark_vessel['longitude']
                if 'timestamp' in dark_vessel and 'detection_time' not in dark_vessel:
                    dark_vessel['detection_time'] = dark_vessel['timestamp']
    ...
    def _has_matching_ais(self, sar_detection: Dict, ais_data: List[Dict], 
        ...
        sar_lat = sar_detection.get('lat', sar_detection.get('latitude'))
        sar_lon = sar_detection.get('lon', sar_detection.get('longitude'))
        sar_time_str = sar_detection.get('detection_time', sar_detection.get('timestamp', ''))
        sar_time = datetime.fromisoformat(sar_time_str.replace('Z', ''))
        
        for ais_vessel in ais_data:
            try:
                # Handle different field name formats for AIS
                ais_lat = float(ais_vessel.get('lat', ais_vessel.get('latitude')))
                ais_lon = float(ais_vessel.get('lon', ais_vessel.get('longitude')))
