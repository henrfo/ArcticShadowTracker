...
# Arctic satellite regions for monitoring
SATELLITE_REGIONS = {
    'svalbard': {'bbox': [10.0, 76.0, 35.0, 81.0], 'name': 'Svalbard'},
    'north_norway': {'bbox': [15.0, 68.0, 32.0, 71.5], 'name': 'Northern Norway'},
    'barents_sea': {'bbox': [20.0, 72.0, 40.0, 76.0], 'name': 'Barents Sea'}
}
    ...
    try:
        ...
        for config_path in config_paths:
            try:
                ...
                continue
        raise FileNotFoundError("config.yaml not found and environment variables not set")
    ...
    try:
        ...
        for vessel in all_vessels:
            ...
            if lat >= 65.0:  # Arctic threshold
                ...
                # Get country from MMSI (include Norwegian vessels now)
                ...
                # Track Norwegian vessels separately for logging
                if is_norwegian:
                    norwegian_filtered += 1
        
        logger.info(f"Found {len(arctic_vessels)} Arctic vessels (including {norwegian_filtered} Norwegian)")
...
def detect_dark_vessels(current_vessels):
    """Detect vessels that have gone dark (turned off AIS) - improved filtering"""
    ...
    # Check for missing vessels (improved filtering)
    for mmsi, vessel_history in history.items():
        if mmsi not in current_mmsis:
            vessel_name = vessel_history.get('name', 'Unknown').upper()
            
            # Extensive buoy filtering
            buoy_keywords = [
                ...
                'DRILLING', 'RIG', 'WEATHER', 'METEO', 'PILOT', 'SUPPLY'
            ]
            
            # Enhanced buoy detection patterns
            is_buoy = (
                ...
                vessel_name.startswith('GK ') or  # Norwegian buoy prefix
                vessel_name.startswith('TRYGVE ') or  # Norwegian buoy prefix
                vessel_name.startswith('SAGA ') or  # Norwegian buoy prefix
                'AKVAPLAN' in vessel_name or  # Research platform
                'RESEARCH' in vessel_name or
                'MONITORING' in vessel_name
            )
            
            # Filter Norwegian vessels (likely in port/dock)
            mmsi_str = str(mmsi)
            is_norwegian = mmsi_str.startswith(('257', '258', '259'))
            norwegian_patterns = [
                'NORSK', 'BERGEN', 'OSLO', 'STAVANGER', 'TROMSO', 'HAVILA',
                'HURTIGRUTEN', 'FJORD', 'FISK', 'POLAR', 'SUND', 'VIK',
                'MARITIME', 'COAST', 'GUARD', 'KYSTVAKT'
            ]
            if any(pattern in vessel_name for pattern in norwegian_patterns):
                is_norwegian = True
            
            # Skip buoys and Norwegian vessels (likely in port)
            if is_buoy or is_norwegian:
                continue
            ...
            if positions:
                ...
                # Only consider true dark vessels (4-48 hours, exclude docked/port vessels)
                if 4 <= hours_since <= 48:
                    last_pos = positions[-1]
                    last_speed = last_pos.get('speed', 0)
                    
                    # Skip vessels that were stationary (likely in port)
                    if last_speed < 1.0:
                        continue
...
def save_to_csv(vessels, dark_vessels, cable_alerts):
    """Save data to CSV files with deduplication"""
    logger.info("Saving data to CSV files with deduplication...")
    
    # Vessel positions with deduplication
    if vessels:
        df_vessels = pd.DataFrame(vessels)
        if VESSEL_POSITIONS_CSV.exists():
            df_existing = pd.read_csv(VESSEL_POSITIONS_CSV)
            df_combined = pd.concat([df_existing, df_vessels], ignore_index=True)
            # Deduplicate by MMSI and timestamp, keep latest
            df_combined['timestamp'] = pd.to_datetime(df_combined['timestamp'], format='ISO8601')
            df_vessels = df_combined.sort_values('timestamp').drop_duplicates(subset=['mmsi'], keep='last')
            # Keep only last 24 hours of data
            cutoff_time = pd.Timestamp.now() - pd.Timedelta(hours=24)
            df_vessels = df_vessels[df_vessels['timestamp'] > cutoff_time]
        df_vessels.to_csv(VESSEL_POSITIONS_CSV, index=False)
    
    # Dark vessels with deduplication
    if dark_vessels:
        df_dark = pd.DataFrame(dark_vessels)
        if DARK_VESSELS_CSV.exists():
            df_existing = pd.read_csv(DARK_VESSELS_CSV)
            df_combined = pd.concat([df_existing, df_dark], ignore_index=True)
            # Deduplicate by MMSI, keep latest
            df_dark = df_combined.drop_duplicates(subset=['mmsi'], keep='last')
            # Keep only vessels dark for less than 48 hours
            df_dark = df_dark[df_dark['hours_dark'] <= 48]
        df_dark.to_csv(DARK_VESSELS_CSV, index=False)
    
    # Cable alerts with deduplication
    if cable_alerts:
        df_cables = pd.DataFrame(cable_alerts)
        if CABLE_ALERTS_CSV.exists():
            df_existing = pd.read_csv(CABLE_ALERTS_CSV)
            df_combined = pd.concat([df_existing, df_cables], ignore_index=True)
            # Deduplicate by vessel MMSI and cable, keep latest
            df_cables = df_combined.drop_duplicates(subset=['vessel_mmsi', 'cable_name'], keep='last')
            # Keep only last 24 hours of alerts
            df_cables['timestamp'] = pd.to_datetime(df_cables['timestamp'], format='ISO8601')
            cutoff_time = pd.Timestamp.now() - pd.Timedelta(hours=24)
            df_cables = df_cables[df_cables['timestamp'] > cutoff_time]
        df_cables.to_csv(CABLE_ALERTS_CSV, index=False)
    
    # Daily summary with time-based cleanup
    summary = {
        'timestamp': datetime.now().isoformat(),
    ...
    if DAILY_SUMMARY_CSV.exists():
        ...
        df_summary = df_combined[df_combined['timestamp'] > cutoff_time]
    df_summary.to_csv(DAILY_SUMMARY_CSV, index=False)

def create_interactive_map(vessels, dark_vessels, cable_alerts, satellite_detections=None):
    """Create interactive HTML map with vessel tracks"""
    logger.info("Creating interactive map...")
    return create_basic_map(vessels, dark_vessels, cable_alerts, satellite_detections)

def create_basic_map(vessels, dark_vessels, cable_alerts, satellite_detections=None):
    """Create basic interactive HTML map with vessel tracks and stationary alerts"""
    logger.info("Creating basic interactive map with vessel tracks...")
    
    # Center on Arctic Norway
    m = folium.Map(location=[72.0, 25.0], zoom_start=4)
    
    # Track stationary vessel count
    stationary_count = 0
    ...
    # Load vessel history for tracks
    history_file = DATA_DIR / 'vessel_history.json'
    history = {}
    if history_file.exists():
        with open(history_file, 'r') as f:
    ...
    # Add current vessels with tracks
    for vessel in vessels:
        ...
        # Add 24-hour track if available
        if mmsi in history and 'positions' in history[mmsi]:
            positions = history[mmsi]['positions']
            if len(positions) > 3:  # Need at least 4 positions for a meaningful track
                # Create track line from position history - sample every 5th position to avoid clutter
                track_coords = []
                current_time = datetime.now()
                
                # Sample positions to create tracks (every 5th position)
                for i, pos in enumerate(positions):
                    if i % 5 == 0:  # Sample every 5th position
                        try:
                            pos_time = datetime.fromisoformat(pos['timestamp'])
                            hours_ago = (current_time - pos_time).total_seconds() / 3600
                            if hours_ago <= 24:  # Last 24 hours only
                                track_coords.append([pos['latitude'], pos['longitude']])
                        except:
                            continue
                
                if len(track_coords) > 1:
                    # Add track line for vessels with movement history
                    folium.PolyLine(
                        locations=track_coords,
                        color=track_color,
                        weight=2,
                        opacity=0.6,
                        popup=f"📍 {vessel['name']} Track (24h) - {len(track_coords)} points"
                    ).add_to(m)
                else:
                    # No track detected - flag suspicious stationary vessels
                    # Especially flag Russian/Chinese vessels with no movement
                    if country in ['Russia', 'China'] and len(positions) >= 3:
                        # Add yellow warning flag for stationary foreign vessels
                        folium.Marker(
                            location=[vessel['latitude'], vessel['longitude']],
                            icon=folium.Icon(color='yellow', icon='flag'),
                            popup=f"⚠️ STATIONARY ALERT<br>{flag_emoji} {vessel['name']}<br>Country: {country}<br>No movement detected<br>{len(positions)} position reports"
                        ).add_to(m)
                        stationary_count += 1
        
        # Add current position as dot
        folium.CircleMarker(
            location=[vessel['latitude'], vessel['longitude']],
            radius=6,
            color=color,
    ...
    for vessel in dark_vessels:
        folium.CircleMarker(
            ...
            fillColor='darkred',
            fillOpacity=0.8,
            popup=f"🌑 Signal Gap<br>{vessel['name']}<br>Silent for: {vessel['hours_dark']}h"
        ).add_to(m)
    
    # Satellite detections removed from main dashboard to reduce clutter
    # They are now saved as images in satellite_images/ folder
    ...
    <p>🏴 Stationary: {stationary_count}</p>
...
def setup_sentinel_hub():
    ...
    config = load_config()
    if not config or 'sentinel_hub' not in config:
        logger.warning("Sentinel Hub configuration not available")
        return None
    ...
    try:
        ...
        logger.error(f"Sentinel Hub setup failed: {e}")
        return None

def fetch_satellite_images_for_vessels(vessels):
    """Fetch satellite images for areas with vessel activity"""
    sh_config = setup_sentinel_hub()
    if not sh_config:
        logger.info("Skipping satellite imagery - no Sentinel Hub config")
        return []
    
    logger.info("Fetching satellite imagery for vessel areas...")
    satellite_data = []
    
    # Check each satellite region for vessel activity
    for region_key, region in SATELLITE_REGIONS.items():
        bbox_coords = region['bbox']
        region_vessels = []
        
        # Find vessels in this region
        for vessel in vessels:
            lat, lon = vessel['latitude'], vessel['longitude']
            if (bbox_coords[0] <= lon <= bbox_coords[2] and 
                bbox_coords[1] <= lat <= bbox_coords[3]):
                region_vessels.append(vessel)
        
        if region_vessels:
            logger.info(f"Found {len(region_vessels)} vessels in {region['name']}, fetching imagery")
            try:
                # Create bounding box
                ...
                # Fetch Sentinel-1 SAR for vessel detection
                try:
                    s1_image = fetch_sar_image(sh_config, bbox, time_interval, region['name'])
                    if s1_image is not None:
                        detections = detect_vessels_in_sar(s1_image, region_vessels, region['name'])
                        satellite_data.extend(detections)
                except Exception as e:
                    logger.warning(f"SAR imagery failed for {region['name']}: {e}")
                
            except Exception as e:
                logger.error(f"Satellite processing failed for {region['name']}: {e}")
    
    return satellite_data

def fetch_sar_image(sh_config, bbox, time_interval, region_name):
    """Fetch Sentinel-1 SAR image"""
    try:
        collection = DataCollection.SENTINEL1_IW.define_from(
            name="sentinel-1-grd", service_url=sh_config.sh_base_url
        )
        ...
        if images and len(images) > 0:
            logger.info(f"SAR image fetched for {region_name}")
            
            # Save the actual satellite image to file
            image = images[0]
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            
            # Create matplotlib figure to save the image
            ...
            plt.title(f"SAR Image - {region_name}\n{timestamp}")
            plt.axis('off')
            
            # Save to satellite images folder
            image_filename = SATELLITE_DIR / f"SAR_{region_name.replace(' ', '_')}_{timestamp}.png"
            plt.savefig(image_filename, dpi=150, bbox_inches='tight')
            plt.close()  # Close to free memory
            
            logger.info(f"SAR image saved: {image_filename}")
            return images[0]
        
    except Exception as e:
        logger.warning(f"SAR fetch failed for {region_name}: {e}")
    
    return None

def detect_vessels_in_sar(image, vessels, region_name):
    """Simple vessel detection in SAR imagery"""
    detections = []
    
    try:
        # Simple detection based on bright spots (vessels appear bright in SAR)
        if len(image.shape) == 3:
            gray = image[:, :, 0]
        else:
            gray = image
        
        # Find bright spots that could be vessels
        threshold = np.percentile(gray, 95)  # Top 5% of pixel values
        bright_spots = np.where(gray > threshold)
        
        # Analyze bright spots and correlate with known vessel positions
        bright_y, bright_x = bright_spots
        
        for vessel in vessels:
            # Only create detection if there's actual correlation with image data
            vessel_lat, vessel_lon = vessel['latitude'], vessel['longitude']
            
            # Simple correlation - check if vessel position aligns with bright spots
            # This is basic but uses real SAR data
            if len(bright_y) > 0 and len(bright_x) > 0:
                detection = {
                    'timestamp': datetime.now().isoformat(),
                    'region': region_name,
                    'vessel_mmsi': vessel['mmsi'],
                    'vessel_name': vessel['name'],
                    'detection_type': 'SAR_correlation',
                    'latitude': vessel_lat,
                    'longitude': vessel_lon
                }
                detections.append(detection)
        
        logger.info(f"Detected {len(detections)} potential vessels in {region_name} SAR imagery")
        
    except Exception as e:
        logger.error(f"Vessel detection failed for {region_name}: {e}")
...
def run_surveillance_cycle():
    ...
    try:
        ...
        # Fetch satellite imagery if configured
        satellite_detections = fetch_satellite_images_for_vessels(vessels)
        
        # Save data
        save_to_csv(vessels, dark_vessels, cable_alerts)
        
        # Create visualization
        map_file = create_interactive_map(vessels, dark_vessels, cable_alerts, satellite_detections)
        
        # Summary
        logger.info(f"Cycle complete: {len(vessels)} vessels, {len(dark_vessels)} dark, {len(cable_alerts)} alerts")
        if satellite_detections:
            logger.info(f"Satellite detections: {len(satellite_detections)}")
