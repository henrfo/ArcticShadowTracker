...
class FreeArcticAISCollector:
                ...
                while datetime.now() < end_time:
                    try:
                        ...
                        if 'Message' in message:
                            vessel_data = self._normalize_aisstream_vessel(message['Message'])
                            if vessel_data:
    ...
    def _normalize_aisstream_vessel(self, vessel_data: Dict) -> Optional[Dict]:
        """Normalize aisstream.io vessel data."""
        try:
            # aisstream.io message format
            vessel = {
                'mmsi': str(vessel_data.get('UserID', 'unknown')),
                'latitude': float(vessel_data.get('Latitude', 0)),
                'longitude': float(vessel_data.get('Longitude', 0)),
                'speed': float(vessel_data.get('RateOfTurn', 0)),
                'course': float(vessel_data.get('Cog', 0)),
                'timestamp': datetime.now().isoformat(),
                'name': vessel_data.get('ShipName', f"VESSEL_{vessel_data.get('UserID', 'UNK')}"),
            ...
            if vessel['latitude'] == 0 or vessel['longitude'] == 0:
                return None
