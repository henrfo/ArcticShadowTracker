...
def deduplicate_vessels(vessels):
    """
    Remove duplicate vessels based on IMO number or vessel name

    Args:
        vessels: List of vessel dicts

    Returns:
        list: Deduplicated vessel list
    """
    seen_imo = set()
    seen_names = set()
    unique_vessels = []
    ...
    for vessel in vessels:
        ...
        # Skip if IMO is known and duplicate
        if imo and imo != 'Unknown' and imo in seen_imo:
            continue

        # Skip if name is duplicate (for vessels without IMO)
        if imo == 'Unknown' or not imo:
...
def main():
    ...
    # Extract from Greenpeace PDF
    print("📂 Source 1: Greenpeace Baltic Tankers PDF")
    pdf_vessels = extract_greenpeace_pdf()
    all_vessels.extend(pdf_vessels)
    print()

    # Add known 2025 vessels
    print("📂 Source 2: Known 2025 Shadow Fleet Vessels")
