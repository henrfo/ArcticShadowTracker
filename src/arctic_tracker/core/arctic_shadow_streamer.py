...
def detect_dark_vessels(current_vessels):
    ...
    # Check for missing vessels (exclude buoys from dark vessel detection)
    for mmsi, vessel_history in history.items():
        if mmsi not in current_mmsis:
            # Filter out buoys from dark vessel detection (improved pattern matching)
            ...
            if is_buoy:
                continue  # Skip buoys from dark vessel detection
...
def create_interactive_map(vessels, dark_vessels, cable_alerts):
    """Create enhanced interactive HTML map with vessel tracks"""
    logger.info("Creating enhanced interactive map with vessel tracks...")
    
    try:
        # Import enhanced dashboard module
        import sys
        sys.path.insert(0, str(Path(__file__).parent.parent))
        from utils.enhanced_dashboard import create_enhanced_dashboard
        
        # Create enhanced dashboard with all vessel tracks
        map_file = create_enhanced_dashboard()
        
        if map_file:
            logger.info(f"Enhanced dashboard created: {map_file}")
            return str(map_file)
        else:
            logger.warning("Enhanced dashboard creation failed, falling back to basic map")
            # Fallback to basic map creation if enhanced fails
            return create_basic_map(vessels, dark_vessels, cable_alerts)
...
def create_basic_map(vessels, dark_vessels, cable_alerts):
    """Create basic interactive HTML map (fallback)"""
    logger.info("Creating basic interactive map...")
