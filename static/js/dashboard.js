...
(function () {
  ...
  // --------------------------------------------------------------------------
  // AnalysisOverlay — manages the full-screen SAR Analysis iframe and the
  // nav badge showing current tile count.
  // --------------------------------------------------------------------------
  const AnalysisOverlay = (function () {
    const analysisBtn = document.getElementById('nav-open-analysis');
    const overlay = document.getElementById('analysis-overlay');
    const frame = document.getElementById('analysis-frame');
    const badge = document.getElementById('analysis-tile-badge');
    let loaded = false;
    ...
    async function fetchTileCount() {
      try {
        const res = await fetch('/api/satellite-tiles');
        if (!res.ok) return;
        const data = await res.json();
        updateBadge((data.tiles || []).length);
      } catch (err) {
        console.error('[AnalysisOverlay] badge fetch failed:', err);
      }
    }

    function init() {
      if (analysisBtn) analysisBtn.addEventListener('click', open);
      document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && isOpen()) close();
      });
      window.addEventListener('message', (e) => {
        if (e && e.data && e.data.type === 'close-analysis') close();
      });
      fetchTileCount();
    }

    return { init, open, close, fetchTileCount };
  })();
